...
/*
 ...
 function onImgClick(e){
    const imgObj = data.find((item) => item.thumbnailUrl == e.target.src);
    console.log(imgObj);
     const imgItem = data.filter(item => item.thumbnailUrl)
     .map(item => { return {
         blgView: item.url,
         smallView: item.thumbnailUrl
       }
      });
     console.log(imgItem);
  }
 ...
 /*
 function showbigImg(value){
     const bigImg = document.createElement('img');
    bigImg.src = value.url;
    imgWrapper.append(bigImg);
    bgFonImg.classList.add('show');
